...
HEADER=$(./scripts/gen_auth_header.sh PUT "http://localhost:7624/mirror" "$TMPFILE")
curl -s -X PUT -H "$HEADER" -H "Content-Type: application/json" -d @"$TMPFILE" http://localhost:7624/mirror | jq '.'
rm -f "$TMPFILE"
